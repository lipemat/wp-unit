...
", \implode( "\n", \array_map( '\rtrim', \explode( "\n", $snapshot->get_snapshot() ) ) ) );
	...
	public function test_format_data(): void {
		...
		$data = [ 'foo' => 'bar', 'empty' => false, 'null' => null, 'array' => [ 'foo' => 'bar' ] ];
		$formatted = PrivateAccess::in()->call_private_method( $snapshot, 'format_data', [ $data ] );
		$formatted = \implode( "\n", \array_map( '\rtrim', \explode( "\n", $formatted ) ) );
...
', $formatted );
		...
		$with_falsy = PrivateAccess::in()->call_private_method( $snapshot, 'format_data', [ $data, true ] );
		$this->assertSame( "Array (
    'foo' => 'bar',
    'empty' => false,
    'null' => null,
	...
	public function test_get_snapshot_file_path(): void {
		...
		$path = PrivateAccess::in()->call_private_method( $snapshot, 'get_snapshot_file_path' );
